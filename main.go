...
func main() {
	...
	protocol := getEnv("FALLBACK_PROTO", "ws") // Render强制ws
	...
	// 输出节点信息（这里是修改点：domain 默认值优化）
	domain := getEnv("DOMAIN", "KRP111.onrender.com") // ← 修改这里！替换成你 Render 服务名.onrender.com
	if domain == "zh-reality-proxy-1.onrender.com" {
		log.Println("INFO: Using default DOMAIN. If wrong, set DOMAIN env var in Render dashboard.")
	}
	...
	var link string
	if protocol == "ws" {
		link = fmt.Sprintf("vless://%s@%s:443?type=ws&security=none&path=%s&host=%s#Render-WS-Node",
			uuid, domain, path, fakeDomain)
	} else {
		// Reality 备用（不推荐在 Render）
		link = fmt.Sprintf("vless://%s@%s:443?security=reality&fp=%s&pbk=%s&sni=%s&sid=%s#Render-Reality-Node",
